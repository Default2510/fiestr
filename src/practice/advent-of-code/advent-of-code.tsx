...
const scenarioCommand = (action: AdventOfCodeAction): string => {
  const scenario: string = 'year' + action.year + 'day' + action.day + (action.part === 1 ? "" : "_2");
...
export const AdventOfCode = (props: AdventOfCodeProps) => {
  ...
  const result2 = scenarioCommand({ year: props.year, day: props.day, part: 2 });

  return (
    <>
      <h1 style={{ fontWeight: 'bold', fontSize: '24px', marginBottom: '10px' }}>Advent of Code {props.year}</h1>
